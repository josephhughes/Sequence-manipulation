...
my ($help,$first,$last,$trim,$in,$out);
&GetOptions(
...
if (($help)||!$in||!$out){
  ...
  print "usage: TrimAlignment [-h] [-f N] [-l N] [-t N] [-z] [-v] [-i INFILE] [-o OUTFILE]\n";
  print "Part of Sequence-manipulation by J. Hughes (joseph(dor)hughes(at)glasgow(dot)ac(dot)uk\n\n";
 ...
 while ( my $aln = $inaln->next_aln ) { 
  # Manipulate
  #$aln->remove_seq($seq);
  my $mini_aln = $aln->slice($first,$last);  # get a block of columns
